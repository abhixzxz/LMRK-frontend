...
function ReportChangingofNom() {
  ...
  const calculateChartStatistics = useCallback((data) => {
    ...
    console.log('📊 Chart calculation - Found keys:', { statusKey, regPersonsKey });
    ...
    console.log('📊 Chart statistics calculated:', { registeredMembers, attendedMembers });
  ...
  const handleSearch = useCallback(async () => {
    try {
      ...
      const response = await fetch('http://localhost:4000/api/regMembersReport', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        if (reportResults.length === 0) {
          setError(`No data found for the selected criteria. Please check if there are ${formData.selectedOption === 1 ? 'admitted' : 'registered'} members for programme "${formData.programmeName}" in time slot "${formData.timeSlots}".`);
  ...
  }, [formData.programmeName, formData.timeSlots, formData.selectedOption, calculateChartStatistics]);
  ...
  const exportToPDF = useCallback(() => {
    ...
    const reportTitle = formData.selectedOption === 1 ? 'Admitted Members Report' : 'Register Members Report';
    
    // Get column headers
    const headers = filteredReportData.length > 0 ? Object.keys(filteredReportData[0]) : [];
    ...
    printWindow.document.write(`
      <html>
        ...
        <body>
          ...
          <div class="report-info">
            ...
            <div class="info-section">
              <div class="info-item">
                ...
                <span class="info-value">${formData.selectedOption === 1 ? '✅ Admitted Members' : '📝 Register Members'}</span>
              </div>
              <div class="info-item">
          ...
          <div class="summary">
            <strong>📋 Report Summary:</strong> This report contains ${filteredReportData.length} records of ${formData.selectedOption === 1 ? 'admitted' : 'registered'} members for the "${formData.programmeName}" programme during "${formData.timeSlots}" time slot.
          </div>
              ...
              <tbody>
                ${filteredReportData.map(row => 
                  `<tr>${Object.values(row).map(value => 
                    `<td>${value !== null && value !== undefined ? String(value).replace(/'/g, '&#39;') : '<span class="no-data">-</span>'}</td>`
                  ).join('')}</tr>`
                ).join('')}
              </tbody>
            </table>
  ...
  }, [filteredReportData, formData.programmeName, formData.timeSlots, formData.selectedOption]);
        ...
        <Box>
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
            ...
            <Typography variant="body2" sx={{ mr: 1 }}>
              {formData.selectedOption === 1 ? 'Admitted Members:' : 'Registered Members:'}
            </Typography>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            />
            <Typography variant="body2" sx={{ mr: 1 }}>
              {formData.selectedOption === 1 ? 'Attended Members:' : 'Other Members:'}
            </Typography>
            <Typography variant="body2" sx={{ fontWeight: 'bold' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: '4px',
                        border: '2px solid',
                        borderColor: formData.selectedOption === 1 ? '#1976d2' : '#e0e0e0',
                        backgroundColor: formData.selectedOption === 1 ? '#1976d2' : 'white',
                    ...
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: '4px',
                        border: '2px solid',
                        borderColor: formData.selectedOption === 0 ? '#1976d2' : '#e0e0e0',
                        backgroundColor: formData.selectedOption === 0 ? '#1976d2' : 'white',
          ...
          <CardContent>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
              <Typography variant="h6" sx={{ color: '#1976d2', fontWeight: 'bold' }}>
                📄 {formData.selectedOption === 1 ? 'Admitted Members' : 'Register Members'} Report ({filteredReportData.length} records)
              </Typography>
            </div>
              ...
              <table style={{
                ...
                <thead>
                  <tr style={{ backgroundColor: '#f5f5f5', position: 'sticky', top: 0, zIndex: 1 }}>
                    {filteredReportData.length > 0 && Object.keys(filteredReportData[0]).map((key, index) => (
                      <th key={index} style={{
                        padding: '12px 8px',
                        textAlign: 'left',
            ...
            <div style={{ 
              ...
              <strong>Report Details:</strong> Programme: {formData.programmeName} | Time Slot: {formData.timeSlots} | Type: {formData.selectedOption === 1 ? 'Admitted Members' : 'Register Members'}
            </div>
          </CardContent>
        </Card>
      )}
