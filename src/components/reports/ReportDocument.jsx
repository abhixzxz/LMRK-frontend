...
function ReportDocument() {
  ...
  const [chartData, setChartData] = useState({ registeredMembers: 0, attendedMembers: 0 }); // Chart statistics
  ...
  const chartStatistics = useMemo(() => {
    ...
    const regPersonsKey = Object.keys(firstRow).find(key =>
    ...
    if (statusKey && regPersonsKey) {
      reportData.forEach(row => {
        ...
        if (String(status).toUpperCase() === 'R') { // Registered
          registeredMembers += regPersons;
        } else if (String(status).toUpperCase() === 'A') { // Attended
  ...
  const fetchProgrammeNames = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:4000/api/ProgrammeName');
  ...
  const fetchTimeSlots = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:4000/api/TimeSlots');
      const data = await response.json();
  ...
  const fetchRowStylingRules = async () => {
    try {
      const response = await fetch('http://localhost:4000/api/rowStyling');
      const data = await response.json();
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      if (!formData.programmeName || !formData.timeSlots) {
        setError('Please select both Programme Name and Time Slots for best results');
        setLoading(false);
        return;
      }
      ...
      const response = await fetch('http://localhost:4000/api/reportdocument', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        reportResults.forEach((row, index) => {
          ...
          const personKeys = Object.keys(row).filter(key =>
            key.toLowerCase().includes('regpersons') || 
            key.toLowerCase().includes('reg_persons') ||
            key.toLowerCase().includes('atnpersons') || 
            key.toLowerCase().includes('atn_persons')
  ...
  const handleAddButtonClick = async (rowIndex, rowData) => {
    ...
    if (isAttended) {
      setError('This member is already marked as attended and cannot be updated.');
      return;
    }
    ...
    const phoneKeys = Object.keys(rowData).filter(key =>
    ...
    try {
      ...
      const response = await fetch('http://localhost:4000/api/updateAttendMember', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        window.alert(`Successfully updated attendance for phone: ${phone}, AtnPersons: ${atnPersons}, name '${name}'`);
  ...
  const getRowStyling = (row) => {
    ...
    const regPersonsKeys = Object.keys(row).filter(key =>
      key.toLowerCase().includes('regpersons') || key.toLowerCase().includes('reg_persons')
    );
    const atnPersonsKeys = Object.keys(row).filter(key =>
      key.toLowerCase().includes('atnpersons') || key.toLowerCase().includes('atn_persons')
    );
    ...
    const regPersons = regPersonsKeys.length > 0 ? parseInt(row[regPersonsKeys[0]]) || 0 : 0;
    const atnPersons = atnPersonsKeys.length > 0 ? parseInt(row[atnPersonsKeys[0]]) || 0 : 0;
  ...
  const PieChart = ({ registeredMembers, attendedMembers }) => {
      ...
      return (
        <Box sx={{ textAlign: 'center', py: 3 }}>
          <Typography variant="body2" color="textSecondary">
            No member data available. Please search for report data to see statistics.
          </Typography>
        </Box>
      );
    }
    ...
    const registeredX = centerX + radius * Math.cos((registeredEndAngle - 90) * Math.PI / 180);
    const registeredY = centerY + radius * Math.sin((registeredEndAngle - 90) * Math.PI / 180);

    const registeredLargeArc = registeredAngle > 180 ? 1 : 0;
    const registeredPath = `M ${centerX} ${centerY} L ${centerX} ${centerY - radius} A ${radius} ${radius} 0 ${registeredLargeArc} 1 ${registeredX} ${registeredY} Z`;

    // Calculate path for attended members (red slice)
    const attendedStartAngle = registeredAngle;
    const attendedStartX = centerX + radius * Math.cos((attendedStartAngle - 90) * Math.PI / 180);
    const attendedStartY = centerY + radius * Math.sin((attendedStartAngle - 90) * Math.PI / 180);

    const attendedLargeArc = attendedPercentage > 50 ? 1 : 0;
    const attendedPath = `M ${centerX} ${centerY} L ${attendedStartX} ${attendedStartY} A ${radius} ${radius} 0 ${attendedLargeArc} 1 ${centerX} ${centerY - radius} Z`;

    return (
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
        <Box sx={{ position: 'relative' }}>
          <svg width="160" height="160" viewBox="0 0 160 160" style={{ border: '1px solid #ddd', borderRadius: '8px' }}>
        ...
        <Box>
          ...
          <Box sx={{ mt: 1 }}>
            <Typography variant="caption" sx={{ color: '#666', fontWeight: '900' }}>
              Total: {total.toLocaleString()} | Green: {registeredPercentage.toFixed(1)}% | Red: {attendedPercentage.toFixed(1)}%
            </Typography>
          </Box>
        </Box>
      </Box>
  ...
  const handleMobileClick = useCallback((mobileNumber) => {
    ...
    <Box className="report-document-container">
      ...
      <Card className="report-form-card" sx={{ mb: 3 }}>
        <CardContent>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '1.5rem', padding: '1rem' }}>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                  <label className="dropdown-label" style={{ fontWeight: 'bold', color: '#555', fontSize: '0.75rem' }}>
                    📋 Programme Name:
                  </label>
                  <select
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                    ...
                    {reportData.length > 0 && (
                      <span style={{
                        ...
                        {filteredReportData.length} of {reportData.length} records
                      </span>
                    )}
                  </div>
                  <div style={{ position: 'relative' }}>
                    <input
                      ...
                      style={{
                        ...
                        padding: '0.35rem 8px',
                        borderRadius: '4px',
                        border: formData.searchText ? '2px solid #1976d2' : '1px solid #e0e0e0',
                        fontSize: '0.75rem',
                        backgroundColor: formData.searchText ? '#f3f8ff' : 'white'
                      }}
                    />
                    {formData.searchText && (
                      <button
...
{rowStylingRules && (
              <Alert severity="info" sx={{ mb: 2 }}>
                <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '1rem', fontSize: '0.875rem' }}>
                  <strong>🎨 Row Colors:</strong>
                  {rowStylingRules.rules.map((rule, index) => (
                    <span key={index} style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                      <div style={{
                        width: '16px',
                        height: '16px',
                        backgroundColor: rule.color,
                      ...
                      <span style={{ fontSize: '0.8rem' }}>
                        {rule.name === 'mismatchAttended' ? '🔵 Mismatch' :
                         rule.name === 'attended' ? '🔴 Attended' :
                         rule.name === 'registered' ? '🟢 Registered' : rule.name}
                      </span>
                    </span>
                  ))}
                  <span style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', marginLeft: '1rem', paddingLeft: '1rem', borderLeft: '1px solid #ddd' }}>
                    <span style={{ fontSize: '0.8rem', color: '#666' }}>
                      💡 <strong>Note:</strong> "Add" button is disabled for attended members (Status: A)
                    </span>
                  </span>
                </div>
              </Alert>
...
<TableHead>
                ...
                <TableBody>
                  ...
                  {filteredReportData.slice(0, 100).map((row) => {
                    ...
                    const status = statusKeys.length > 0 ? row[statusKeys[0]] : '';
                    const isAttended = String(status).toUpperCase() === 'A';
                      ...
                      >
                        <TableCell>
                          <button
                            ...
                            onClick={() => handleAddButtonClick(originalIndex, row)}
                            disabled={isAttended}
                            style={{
                            ...
                            title={isAttended ? 'Already attended - cannot update' : 'Click to update attendance'}
                          >
                            {isAttended ? '✓ Done' : 'Add'}
                          </button>
                        </TableCell>
                        <TableCell>
                          <input
                            ...
                            onChange={(e) => handleAtnPersonsChange(originalIndex, e.target.value)}
                            placeholder={isAttended ? "Already attended" : "Enter Atnpersons"}
                            disabled={isAttended}
                            style={{
                              width: '120px',
                            ...
                            title={isAttended ? 'Cannot modify - already attended' : 'Enter number of persons attending'}
                          />
                        </TableCell>
                        {Object.entries(row)
            ...
            {filteredReportData.length > 100 && (
              <Alert severity="info" sx={{ mt: 1 }}>
                ⚡ Performance optimization: Showing first 100 of {filteredReportData.length} records.
                Use search to narrow down results for better performance.
              </Alert>
            )}
          </CardContent>
        </Card>
      ...
      {/* No Results Message */}
      {!loading && reportData.length > 0 && filteredReportData.length === 0 && debouncedSearchText && (
        <Alert severity="info">
          No records found matching "{debouncedSearchText}". Try a different search term.
        </Alert>
      )}

      {!loading && reportData.length === 0 && formData.programmeName && formData.timeSlots && !error && (
        <Alert severity="info">
                  ...
                  {!isScanning ? (
                    <Box sx={{ mb: 3 }}>
                      ...
                      <Typography variant="body2" sx={{ mb: 2, color: '#666' }}>
                        Click "Start Scanning" to open your camera and scan a QR code
                      </Typography>
                      <Button
                        variant="contained"
                        onClick={startQRScanner}
            ...
            <Box>
              ...
              <Card sx={{ mb: 2, backgroundColor: '#f8f9fa' }}>
                <CardContent>
                  ...
                  {/* Parsed Data */}
                  {qrScanResult.parsedData && typeof qrScanResult.parsedData === 'object' && (
                    <Box>
                      ...
                      <Box sx={{ 
                        ...
                        {Object.entries(qrScanResult.parsedData).map(([key, value]) => {
                          const isMobileNumber = /^(\+?\d{1,3}[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}$/.test(String(value)) ||
